...
class JobManager:
    ...
    async def _run_scrape_job(self, job_id: str) -> None:
        ...
        try:
            ...
            job['status'] = JobStatus.FAILED.value
            job['message'] = f"Scraping failed: {str(e)[:200]}"
        finally:
            job['updated_at'] = datetime.utcnow().isoformat()
            self._save_jobs()
    ...
    async def _run_generate_audio_job(self, job_id: str) -> None:
        ...
        for idx, text_file in enumerate(text_files):
            ...
            # Run audio generation using main codebase
            from src.tts.generator import AudioGenerator
            
            def run_generator():
                generator = AudioGenerator()
                return generator.generate_chapter_chunked(
                    text_path=text_file,
                    output_dir=None,  # Use default (same directory as text)
                    chunk_duration_minutes=1.0,
                    speaker=job['parameters'].get('speaker'),
                    language=job['parameters'].get('language'),
                    speed=job['parameters'].get('speed'),
                    emotion=job['parameters'].get('emotion'),
                    chapter_title=chapter_title,
                    text_offset=0,
                )
            
            try:
                audio_files = await asyncio.to_thread(run_generator)
                logger.info(f"Generated {len(audio_files)} chunks for {text_file.name}")
            except Exception as e:
                logger.error(f"Error generating audio for {text_file.name}: {e}", exc_info=True)
    ...
    async def _run_generate_chapter_audio_job(self, job_id: str) -> None:
        ...
        # Import generator from main codebase
        from src.tts.generator import AudioGenerator
        
        job['message'] = 'Generating audio...'
        self._save_jobs()
        
        # Run generator in thread pool (it's CPU/GPU intensive and blocking)
        try:
            def run_generator():
                generator = AudioGenerator()
                return generator.generate_chapter_chunked(
            ...
            job['status'] = JobStatus.COMPLETED.value
            job['message'] = f'Audio generation completed: {len(audio_files)} chunks'
            job['progress'] = 100
        except Exception as e:
            logger.error(f"Audio generation job {job_id} failed: {e}", exc_info=True)
            job['status'] = JobStatus.FAILED.value
            job['message'] = f"Generation failed: {str(e)[:200]}"
        finally:
            job['updated_at'] = datetime.utcnow().isoformat()
            self._save_jobs()
    ...
    async def _run_generate_chunk_audio_job(self, job_id: str) -> None:
        ...
        try:
            def run_generator():
                generator = AudioGenerator()
                # generate_chapter_chunked skips existing chunks, so it will only generate the missing one
                return generator.generate_chapter_chunked(
                    text_path=text_file,
                    output_dir=None,  # Use default (same directory as text)
                    chunk_duration_minutes=1.0,
            ...
            # Check if our specific chunk was generated
